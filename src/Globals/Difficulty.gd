...
func _process(delta: float) -> void:
	...
	if elapsed_time >= difficulty_increase_seconds:
		speed_modifier = min(speed_modifier + difficulty_increase, SPEED_MODIFIER_MAX)
		spawn_modifier = max(spawn_modifier - difficulty_increase, SPAWN_MODIFIER_MIN)
